...
def email_citizens_account_validated(db,mandrill)
	...
	#threads.each {|t| t.join}
	File.write("20161229_email_shoot_account_validated_2.txt",JSON.dump(results))
end

def email_citizens_birthday_corrected(db,mandrill)
	#get_citizens="SELECT user_key,email,firstname,lastname, 3 as missing FROM users WHERE email IS NOT NULL AND email='tfavre@gmail.com' LIMIT 2"
	get_citizens="select u.email,u.user_key from users as u where tags @> ARRAY['birthday_issue']"
	subject="Bonne nouvelle : votre souci a été corrigé, vous pouvez voter !"
	res_citizens=db.exec(get_citizens)
	if not res_citizens.num_tuples.zero? then
		emails=[]
		res_citizens.each do |r|
	...
	emails.each do |k|
		#threads << Thread.new do
			begin
				result=mandrill.messages.send_template("laprimaire-org-souci-birthday-corrected",[],k)
				puts "sending email to #{k[:to][0][:email]} #{result.inspect}"
				results.push({:email=>result[0]['email'], :status=>result[0]['status'],:reject_reason=>result[0]['reject_reason'],:id=>result[0]['_id']})
			rescue Mandrill::Error => e
				msg="A mandrill error occurred: #{e.class} - #{e.message}"
	...
	#threads.each {|t| t.join}
	File.write("20161229_email_birthday_corrected.txt",JSON.dump(results))
end

def email_citizens_last_chance(db,mandrill)
	#get_citizens="SELECT user_key,email,firstname,lastname, 3 as missing FROM users WHERE email IS NOT NULL AND email='tfavre@gmail.com' LIMIT 2"
	get_citizens="select email,user_key,validation_level from users where not tags @> ARRAY['voteur2'] and email_status=2 and validation_level=3"
	subject="DERNIER RAPPEL : Votez avant la cloture des votes ce soir à 20h"
	res_citizens=db.exec(get_citizens)
	if not res_citizens.num_tuples.zero? then
		emails=[]
		res_citizens.each do |r|
	...
	emails.each do |k|
		#threads << Thread.new do
			begin
				result=mandrill.messages.send_template("laprimaire-org-2nd-tour-de-vote-mini-final",[],k)
				puts "sending email to #{k[:to][0][:email]} #{result.inspect}"
				results.push({:email=>result[0]['email'], :status=>result[0]['status'],:reject_reason=>result[0]['reject_reason'],:id=>result[0]['_id']})
			rescue Mandrill::Error => e
				msg="A mandrill error occurred: #{e.class} - #{e.message}"
